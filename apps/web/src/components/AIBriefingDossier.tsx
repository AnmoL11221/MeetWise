...
export default function AIBriefingDossier({ meetingId }: AIBriefingDossierProps) {
  ...
  return (
    <div className="bg-gray-900/50 border border-gray-700 rounded-lg p-6">
      ...
      <div className="flex flex-wrap gap-2 mb-6 bg-gray-800 rounded-lg p-2">
        {[
      ...
      <div className="space-y-6">
        {activeTab === 'overview' && (
          <div className="space-y-4">
            <div className="bg-gray-800/50 border border-gray-600 rounded-lg p-4 max-h-40 overflow-auto">
              <h3 className="text-lg font-semibold text-white flex items-center gap-2 mb-3">
                <LightbulbIcon className="w-5 h-5 text-yellow-400" />
                Key Insights
              </h3>
              <p className="text-gray-300 break-words whitespace-pre-wrap min-h-6">
                {dossier.keyInsights && dossier.keyInsights.trim().length > 0
                  ? dossier.keyInsights
                  : 'No insights generated yet. Click Regenerate to analyze this meeting.'}
              </p>
            </div>
            ...
            <div className="bg-gray-800/50 border border-gray-600 rounded-lg p-4 max-h-40 overflow-auto">
              ...
              <p className="text-gray-300 break-words whitespace-pre-wrap min-h-6">
                {dossier.recommendations && dossier.recommendations.trim().length > 0
                  ? dossier.recommendations
                  : 'No actionable recommendations yet. Try adding agenda items or regenerate.'}
              </p>
            </div>
            ...
            {dossier.marketNews.length > 0 ? (
              dossier.marketNews.map((news, index) => (
                <div key={index} className="bg-gray-800/50 border border-gray-600 rounded-lg p-4">
                  ...
                  <p className="text-gray-300 break-words whitespace-pre-wrap">{news.summary}</p>
                </div>
              ))
            ) : (
              <div className="text-center py-8">
